...
- 🎯 Entry points to execute your program using `python -m <fastapi_example>` or `$ fastapi_example` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/sorensenNiels/fastapi-example/branch/main/graph/badge.svg?token=fastapi-example_token_here)](https://codecov.io/gh/sorensenNiels/fastapi-example)
[![CI](https://github.com/sorensenNiels/fastapi-example/actions/workflows/main.yml/badge.svg)](https://github.com/sorensenNiels/fastapi-example/actions/workflows/main.yml)

Awesome fastapi_example created by sorensenNiels

## Install

from source
```bash
git clone https://github.com/sorensenNiels/fastapi-example fastapi_example
cd fastapi_example
make install
```
...
```bash
pip install fastapi_example
```

## Executing

```bash
$ fastapi_example run --port 8080
```

or

```bash
python -m fastapi_example run --port 8080
```
...
```bash
❯ fastapi_example --help
Usage: fastapi_example [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_example create-user --help
Usage: fastapi_example create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_example create-user admin admin --superuser
```
...
Run with `fastapi_example run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_example create-user`.
...
-----------------------------------------------------
fastapi_example/__init__.py              4      0   100%
fastapi_example/app.py                  16      1    94%
fastapi_example/cli.py                  21      0   100%
fastapi_example/config.py                5      0   100%
fastapi_example/db.py                   10      0   100%
fastapi_example/models/__init__.py       0      0   100%
fastapi_example/models/content.py       47      1    98%
fastapi_example/routes/__init__.py      11      0   100%
fastapi_example/routes/content.py       52     25    52%
fastapi_example/routes/security.py      15      1    93%
fastapi_example/routes/user.py          52     26    50%
fastapi_example/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_example/default.toml.
